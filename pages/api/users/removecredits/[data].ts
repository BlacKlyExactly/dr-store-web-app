...
export default async ( req: NextApiRequest, res: NextApiResponse ) => {
    return new Promise(async ( resolve ) => {
        ...
        pool.query(
            ...
            ( error ) => {
                if(error) {
                    logger(`Błąd zapytania (usuwanie kredytów | klient: ${steamId}, wartość ${value}) (${scriptName}):${os.EOL} ${JSON.stringify(error)}`);
                ...
                logger(`Pomyślnie usunięto kredyty (klient: ${steamId}, wartość ${value}) (${scriptName}):${os.EOL} ${JSON.stringify(error)}`);
